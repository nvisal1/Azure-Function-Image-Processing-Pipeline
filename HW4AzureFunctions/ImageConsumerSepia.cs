    ...
    public static class ImageConsumerSepia
    {
        ...
        public static async void Run([BlobTrigger(route, Connection = ConfigSettings.STORAGE_CONNECTIONSTRING_NAME)]Stream myBlob, string name, ILogger log)
        {
            ...
            JobEntity initialJobEntity = new JobEntity()
            {
                ...
                ImageConversionMode = "Sepia",
                Status = 1,
                StatusDescription = "Image Obtained",
                ImageSource = $"{Environment.GetEnvironmentVariable(ConfigSettings.STORAGE_DOMAIN_METADATA_NAME)}/{ConfigSettings.TO_SEPIA_CONTAINER_NAME}/{name}",
                ImageResult = "",
            };
